...
public class ConcurrentUpdateManager {
    ...
    public int updateLatestPositionWithLock(gc.mda.signal_batch.model.VesselLatestPosition position) {
     ...
     */
    private int updateLatestPositionWithoutLock(gc.mda.signal_batch.model.VesselLatestPosition position) {
        String sql = """
        INSERT INTO signal.t_vessel_latest_position (
            sig_src_cd, target_id, lat, lon, geom,
            sog, cog, heading, ship_nm, ship_ty,
    ...
    public void batchUpdateWithRowLock(List<gc.mda.signal_batch.model.VesselLatestPosition> positions) {
        // 선박별로 정렬하여 데드락 방지
        ...
        for (gc.mda.signal_batch.model.VesselLatestPosition position : positions) {
            try {
                // Row lock 시도
     ...
     */
    @Transactional(isolation = Isolation.READ_COMMITTED)
    public void mergeTileStatistics(List<gc.mda.signal_batch.model.TileStatistics> statistics) {
        String sql = """
